...
> **实现状态（2026-09-15）**：第一版可执行接口已经收敛为四个 MCP 工具及同源 REST 适配层，参数以 `code/mcp-server/src/server.ts` 和 `code/mcp-server/README.md` 为准。本文其余内容保留为需求背景；手机号登录、JWT 和外部平台真实数据不属于本次模拟数据 MCP 的输入范围。
