...
- `outputs/quality-report/A02_模拟数据与质量报告_v1.2.0.xlsx`：面向团队和答辩的汇总工作簿。
...
三个规模目录继续使用 `v0.2-seed`、`v0.5-core`、`v1.0-full`，但共享新的数据契约 `schema_version=1.2.0`。目录名表示数据规模，不等于 Schema 版本；字段含义发生变化时必须提升 `schema_version`，不能静默修改。
...
- 知识库：`knowledge_cards.csv` 与 `retrieval_eval.csv`。54 张 `role_skill` 卡有直接评分关系；126 张 `role_adjacent_skill` 卡是合成相邻能力扩展，只用于检索和路线探索，不参与匹配评分。
...
- `templates/golden_manual_review_template.csv` 仅为空白模板；团队成员须在 `reviews/golden_manual_review.csv` 中签字，12 条全部通过后才能冻结黄金案例。
- 自动检查数量以 `reports/validation_report.json` 的当次结果为准，不在文档中固定写死。
- 新版完整 ZIP 将在 PR 合并后以 `dataset-v1.2.0` Release 发布；合并前可运行生成器在本地获得完整包。
